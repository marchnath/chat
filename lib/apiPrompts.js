import {
  ...
  getConversationalModeDescription,
} from "./apiHelpers";

// System prompt creation
export function createSystemPrompt(language, proficiencyLevel, contactName) {
  const responseLanguage = getLanguageName(language);
  const proficiencyGuidance = getProficiencyGuidance(proficiencyLevel);
  const conversationalMode = getConversationalModeDescription(contactName);

  return `You are engaging in a ${conversationalMode} conversation as a helpful language partner
